...
void enumerate_files(void){
    ...
    while((dint = readdir(dirp)) != NULL){
        if(dint->d_type == DT_REG){
            snprintf(fname, sizeof(fname), "%s/%s", MOUNT_POINT, dint->d_name);
            if(stat(fname, &st) == 0){
...
esp_err_t init_orig_sd(void){
    ...
    slot_config.width = 1;
    // slot_config.gpio_cd = SDMMC_SLOT_NO_CD; // no card detect
    // slot_config.gpio_wp = SDMMC_SLOT_NO_WP; // no write protect 
    // slot_config.flags |= SDMMC_SLOT_FLAG_INTERNAL_PULLUP; // enable internal pullups on the bus 
    // slot_config.flags |= SDMMC_SLOT_FLAG_WP_ACTIVE_HIGH; // write protect active high
    // slot_config.flags |= SDMMC_SLOT_FLAG_UHS1; // enable UHS-I mode
    // Set the GPIOs used for the card.
    slot_config.clk = GPIO_NUM_16;  // CLK
    slot_config.cmd = GPIO_NUM_6;   // CMD
    slot_config.d0  = GPIO_NUM_7;   // D0
    slot_config.d1  = GPIO_NUM_35;  // D1
    slot_config.d2  = GPIO_NUM_10;  // D2
    slot_config.d3  = GPIO_NUM_9;   // D3
    ...
    slot_config.cd  = GPIO_NUM_NC;   // CD

    slot_config.width = 4;
    slot_config.flags &= ~SDMMC_HOST_FLAG_8BIT;
    ...
    //TOD retry card iniialization if we get a 0x107 or other recoverable error
    // may need some manner of card reset between attempts
    // for extra credit, retrigger iniaiization if card removed & reinserted
    ...
    if (ret != ESP_OK) {
        if (ret == ESP_FAIL) {
            ...
            ESP_LOGE(TAG, "Failed to initialize the card (0x%04x).", ret);
        }
        return -1;
    }
    if ((ret  = esp_vfs_fat_sdmmc_mount("/sdcard", &host, &slot_config, &mount_config, &card)) != ESP_OK) {
        
        ESP_LOGE(TAG, "Failed to initialize the card AGAIN! (0x%04x).", ret);
        return ESP_ERR_NOT_FOUND;
...
//    f = fopen("/sdcard/out.raw", "r");
//
//    if( f == NULL ){
//
//    	printf("Open out.raw failed\n");
//
//    }
    const char root_dir[] = "/sdcard/";
    ...
    while(dint != NULL){
        if(dint->d_type == DT_REG){
            ...
            memcpy(fname, root_dir, 10);
            memcpy(fname + strlen(root_dir), dint->d_name, strlen(dint->d_name));
            fname[8+strlen(dint->d_name)] = '\0';
            ...
            f = fopen(fname, "r");
            if(f == NULL){
                ESP_LOGE(TAG, "Failed to open file %s with errno %d", fname, errno);
            } else {
                ESP_LOGI(TAG, "Opened file %s successfully", fname);
                fclose(f);
            }
