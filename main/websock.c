...
static void shutdown_handler(TimerHandle_t xtimer){
    ESP_LOGI(TAG, "No data received for %d seconds, signalling shutdown", NO_DATA_TIMEOUT_SECS);
...
void websocket_task(void* nada)
{
    ...
    shutdown_signal_timer = xTimerCreate("Websocket shutdown timer", NO_DATA_TIMEOUT_SECS * 1000 / portTICK_PERIOD_MS,
    ...
    /* esp_websocket_unregister_events(client, WEBSOCKET_EVENT_ANY, websocket_event_handler); */ // not in latest example !?
